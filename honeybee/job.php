<!DOCTYPE html>
<!--[if lt IE 7]>      <html class="no-js lt-ie9 lt-ie8 lt-ie7"> <![endif]-->
<!--[if IE 7]>         <html class="no-js lt-ie9 lt-ie8"> <![endif]-->
<!--[if IE 8]>         <html class="no-js lt-ie9"> <![endif]-->
<!--[if gt IE 8]><!-->
<html class="no-js">
<!--<![endif]-->
<head>
<meta charset="utf-8">
<meta http-equiv="X-UA-Compatible" content="IE=edge">
<title>みつばち | メンズエステ | 大阪 | 天神橋六丁目 | 扇町 | 南森町 | 長堀橋</title>
<meta name="viewport" content="width=device-width, initial-scale=1">
<meta name="author" content="Miss Mrs" />

<meta name="description" content="みつばち | メンズエステ | 大阪 | 天神橋六丁目 | 扇町 | 南森町 | 長堀橋" />
<meta name="keywords" content="みつばち | メンズエステ | 大阪 | 天神橋六丁目 | 扇町 | 南森町 | 長堀橋" />

<!-- START:SNS meta-->
<?php include("common/sns_meta.html"); ?>
<!-- END:SNS meta-->

<!-- START:SNS meta-->
<?php include("common/css.html"); ?>
<!-- END:SNS meta-->
</head>
<body>

	<div class="fh5co-loader"></div>
	<div id="page">


		<!-- START:header -->
		<?php include("common/navi.html"); ?>
		<!-- END:header -->
		<div id="page-top"></div>
		<!-- START:Main -->
		<div id="container">
			<h1 class="type1-1">
				<span> セラピストさん大募集!!</span>
			</h1>
			<h2 class="type2-1">
				<span> 求人情報</span>
			</h2>

			<div class="row">
				<div class="col-md-4  text-center fh5co-heading animate-box">
					<article>
						<figure>
							<img src="images/recruit.png" alt="" width="100%">
						</figure>
						<h4>入店祝い金 5万円贈呈</h4>
						<p class="btn-recruit">
							<a href="recruit/index.php">▼ 詳しくは専用サイトへ...▼</a>
						</p>
						<strong>今なら・・・入店祝い金 なんと5万円を贈呈 </strong>


						<!-- START: Job Contact -->
						<?php include("common/job_contact.html"); ?>
						<!-- END: Job Contact  -->
					</article>
				</div>
				<div class="col-md-4  text-center fh5co-heading animate-box">

					<article>
						<figure>
							<img src="images/recruit2.png" alt="" width="100%">
						</figure>
						<h4>ノルマ制なし</h4>
						<p class="btn-recruit">
							<a href="recruit/index.php">▼ 詳しくは専用サイトへ...▼</a>
						</p>

						<strong>出勤日数・売上ノルマなし!! </strong>
						<!-- START: Job Contact -->
						<?php include("common/job_contact.html"); ?>
						<!-- END: Job Contact  -->

					</article>

				</div>
				<div class="col-md-4  text-center fh5co-heading animate-box">
					<article>
						<figure>
							<img src="images/recruit3.png" alt="" width="100%">
						</figure>
						<h4>未経験でもOK</h4>
						<p class="btn-recruit">
							<a href="recruit/index.php">▼ 詳しくは専用サイトへ...▼</a>
						</p>
						<strong>セラピストの80％が未経験からのスタート!! </strong>
						<!-- START: Job Contact -->
						<?php include("common/job_contact.html"); ?>
						<!-- END: Job Contact  -->
					</article>
				</div>
			</div>

			<div id="container">
				<div class="row">
					<div class="col-md-4  text-center fh5co-heading animate-box">
						<article>
							<figure>
								<img src="images/recruit4.png" alt="" width="100%">
							</figure>
							<h4>高額なバック率</h4>
							<p class="btn-recruit">
								<a href="recruit/index.php">▼ 詳しくは専用サイトへ...▼</a>
							</p>
							<strong>完全歩合制の高待遇！バック率50〜70％ </strong>
							<!-- START: Job Contact -->
							<?php include("common/job_contact.html"); ?>
							<!-- END: Job Contact  -->
						</article>
					</div>
					<div class="col-md-4  text-center fh5co-heading animate-box">
						<article>
							<figure>
								<img src="images/recruit5.png" alt="" width="100%">
							</figure>
							<h4>アクセス抜群の勤務地</h4>
							<p class="btn-recruit">
								<a href="recruit/index.php">▼ 詳しくは専用サイトへ...▼</a>
							</p>
							<strong>天神橋6丁目 / 扇町 / 南森町 / 長堀橋</strong>
							
							<!-- START: Job Contact -->
							<?php include("common/job_contact.html"); ?>
							<!-- END: Job Contact  -->
						</article>

					</div>
					<div class="col-md-4  text-center fh5co-heading animate-box">
						<article>
							<figure>
								<img src="images/recruit2.png" alt="" width="100%">
							</figure>
							<h4>完全歩合制度の高待遇</h4>
							<p class="btn-recruit">
								<a href="recruit/index.php">▼ 詳しくは専用サイトへ...▼</a>
							</p>
							<strong>給与システム例</strong>
							<table class="sample_01 mb5">
								<tbody>
									<tr>
										<td>90分</td>
										<td>6,000～8,000円</td>
									</tr>
									<tr>
										<td>120分</td>
										<td>8,000～11,000円</td>
									</tr>
								</tbody>
							</table>
							<p></p>
							<strong>雑費引きなし! ノルマなし!</strong>
							<!-- START: Job Contact -->
							<?php include("common/job_contact.html"); ?>
							<!-- END: Job Contact  -->
						</article>
					</div>
				</div>

				<!--/main-->

			</div>






			<!-- END:Main -->

			<!-- START:footer -->
			<?php include("common/footer.html"); ?>
			<!-- END:footer -->

			<!-- START:footer menu -->
			<?php include("common/footer_menu.html"); ?>
			<!-- END:footer menu-->

		</div>


		<!-- START: goto top -->
		<?php include("common/gototop.html"); ?>
		<!-- END: goto top -->

		<!-- START: Inclide JS -->
		<?php include("common/js.html"); ?>
		<!-- END: Inclide JS -->
	</div>
</body>
</html>